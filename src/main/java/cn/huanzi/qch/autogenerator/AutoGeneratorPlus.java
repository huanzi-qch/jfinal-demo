...
public class AutoGeneratorPlus {
     ...
     * 数据连接相关，需要手动设置
     */
    private static final String URL = "jdbc:mysql://localhost:3306/jfinal_demo?serverTimezone=GMT%2B8&characterEncoding=utf-8";
    ...
    private String basePackage = "cn\\huanzi\\qch\\";//根包位置
                ...
                if(!forFlag && forContent.length() > 0){
                    ...
                    for (TableInfo tableInfo : tableInfos) {
                        String tableColumns = forContent.toString()
                                ...
                                .replaceAll("\\$\\{tableInfo.column_name}", tableInfo.getColumnName())
                                .replaceAll("\\$\\{tableInfo.columnNameFirstToUpperCase}", StringUtil.captureName(StringUtil.camelCaseName(tableInfo.getColumnName())))
                        ...
                        //前补tab，后补换行符
                        stringBuilder.append(tableColumns);
                    }
                    //置空
    ...
    private String create() {
        ...
        writer(tlfPath+"base_model.tlf",
                filePath + "model\\base\\Base" + captureName + ".java",
                tableInfo,customParameter);
        writer(tlfPath+"model.tlf",
                filePath + "model\\" + captureName + ".java",
                tableInfo,customParameter);
